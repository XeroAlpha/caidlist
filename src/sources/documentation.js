...
const { parse: parseHtml, TextNode, HTMLElement, Node } = require("node-html-parser");
...
/** @type {Record<string, (el: import("node-html-parser").Node, target: any) => string | undefined>} */
const BedrockDocStates = {
    "end": function() {},
    "initial": function(el, { meta }) {
        if (el instanceof HTMLElement && el.tagName == "H1") {
            const match = /(.+?)\s*Version:\s*([\d.]+)/.exec(el.innerText.trim());
            meta.title = match[1];
            meta.version = match[2];
            return "index.title";
        }
    },
    "index.title": function(el) {
        if (el instanceof HTMLElement && el.tagName == "H2" && el.innerText.trim() == "Index") {
            return "index.table";
        }
    },
    "index.table": function(el, { meta, state }) {
    ...
    "section": function(el, { state: { indexedSections, sectionStack } }) {
        let content;
    ...
    "table.row": function(el, { columnNames, rows }) {
...
function processDocStateMachine(elements, initialState, target) {
    ...
    for (const element of elements) {
        const stateFunction = BedrockDocStates[state];
...
function generateSchema(content) {
    ...
    if (Array.isArray(content)) {
        ...
        if (!firstRow) {
            throw new Error("Invalid table: " + stack.join(" "));
        }
        ...
        if (isTableType(TableColumnNames.Schema)) {
            ...
            console.warn("Unexpected table: " + stack.join(" "));
        }
    }
    return schema;
}
        ...
        if (typeof schema == "object") {
            ...
            visitSchema(translatedSchema, (type, schemaNode, path) => {
                ...
                if (v) {
                    const match = /\{(.+?)\} (.+)/.exec(v);
                    if (match) {
                        schemaNode.type = match[1];
                        schemaNode.description = match[2];
                    } else {
            ...
            output.push("```json");
            output.push(JSON.stringify(typedJSON, null, 4));
            output.push("```");
